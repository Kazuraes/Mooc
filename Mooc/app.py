...
@app.route('/page') # on fait une route flask avec le mots cle @app.route et on donne un nom
# ⚠ c'est obligatoire pour pouvoir aller sur une page
def page() : 
    return render_template('page.html') # fonction qui retourne la page.html ici il faut mettre rendre_template
...
def inscription() : 
    if request.method == 'POST' : # si la method est post alors
        ...
        # voir index.html la syntexte suivante {% if error %} si erreur alors paragraphe rouge indiquant l'ereur
    ...
    return redirect(url_for('page'))# des que tout est envoier email prenom password alors on renvoie vers la page.html
    ...
    if request.method == 'POST' : # si la method post alors
        ...
        for utilisateur in utilisateurs : # on parcours tout le json
            ...
            if utilisateur.get('email') == email and utilisateur.get('password') == password :
                return redirect(url_for('page')) # alors on renvoie vers page.html
...
@app.route('/creer_une_page_html') # route vers creer_une_page_html
def creer_une_page_html() : 
    return render_template('creer_une_page_html.html.html') # on renvoie vers la creer_une_page_html.html

@app.route('/organiser_votre_texte') # route vers organiser_votre_texte
def organiser_votre_texte () : 
    return render_template('organiser_votre_texte.html') # on renvoie vers la paorganiser_votre_texte.html
...
@app.route('/inserer_des_images_en_html') # route vers inserer_des_images_en_html
def inserer_des_images() :
    return render_template('inserer_des_images.html')
